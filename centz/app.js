...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `PUNKTE: ${state.game.score}`;
		scoreNode.style.display = 'block';
		cubeCountNode.style.opacity = 0.65 ;
	}
	cubeCountNode.innerText = `WÜRFEL ZERSTÖRT: ${state.game.cubeCount}`;
}
...
function renderMenus() {
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			if (isNewHighScore()) {
				highScoreLblNode.textContent = 'NEUER HÖCHSTPUNKTESTAND!';
			} else {
				highScoreLblNode.textContent = `HÖCHSTER PUNKTESTAND: ${formatNumber(getHighScore())}`;
			}
